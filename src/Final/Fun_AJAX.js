...
function ChangeEvent(TeamEvent,whichForm,call,addAll)
{
	...
	{	
		if (XMLHttp.readyState==XHS_COMPLETE || XMLHttp.readyState==XHS_UNINIT)
		{
			...
			XMLHttp.open("POST",WebDir+"Final/ChangeEvent.php?Ev=" + Ev + "&TeamEvent=" + TeamEvent,true);
...
function ChangeEventPhase(TeamEvent,whichForm)
{
	...
	{	
		if (XMLHttp.readyState==XHS_COMPLETE || XMLHttp.readyState==XHS_UNINIT)
		{
			...
			XMLHttp.setRequestHeader("Content-Type","application/x-www-form-urlencoded");
			XMLHttp.onreadystatechange=ChangeEventPhase_StateChange;
...
function move2nextPhase(ev,match,team)
{
	...
	{			
		...
		if (XMLHttp.readyState==XHS_COMPLETE || XMLHttp.readyState==XHS_UNINIT)
		{			
			XMLHttp.open("GET",WebDir+"Final/Move2NextPhase.php?event=" + ev + "&match="  +  match + "&team=" + team + "&ts=" + ts4qs(),true);
// 			document.getElementById('idOutput').innerHTML="../ChangeEvent.php?Ev=" + Ev + "&TeamEvent=" + TeamEvent;
			XMLHttp.setRequestHeader("Content-Type","application/x-www-form-urlencoded");
