	...
	if ($VediGriglia)
	{
		...
		*/
			...
			// Estraggo la griglia della fase $CurPhase
				$Select
					= "SELECT GrPhase,GrPosition,GrMatchNo,EvMatchMode,	/* Grids*/ "
					. "TfNotes,TfTeam,TfMatchNo,TfEvent, IF(EvMatchMode=0,TfScore,TfSetScore) AS Score, TfTie, TfTiebreak,/* Finals*/ "
					. "CoCode,CONCAT(CoName, IF(TfSubTeam>'1',CONCAT(' (',TfSubTeam,')'),'')) as TeamName, /* Countries */ "
				...
				// righe di testa della fase
					...
					// Atleta
						$MyGrid[$Row][$Col].= '<td  nowrap class="' . ($AthPrinted==1 ? 'Bottom ' : '') . 'Top Right Left"><div id="idAth_' . $MyRow->TfEvent . '_' . $MyRow->GrMatchNo . '">' . (!is_null($MyRow->TeamName) ? $MyRow->TeamName : '&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;') /*. ' - ' . $MyRow->GrMatchNo*/ . '</div></td>';
					// Codice Nazione (o bandiera)
						$MyGrid[$Row][$Col].= '<td nowrap class="' . ($AthPrinted==1 ? 'Bottom ' : '') . 'Top Right Left"><div id="idCty_' . $MyRow->TfEvent . '_' . $MyRow->GrMatchNo . '">' . (!is_null($MyRow->CoCode) ? $MyRow->CoCode : '&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;') . '</div></td>';
					// Punteggio
					...
					// tie
						...
						if (isset($_REQUEST['d_Tie']) && $_REQUEST['d_Tie']==1)
						{
							...
							for ($i=0;$i<$obj->so;++$i)
							{
								$MyGrid[$Row][$Col].= '<input  class="disabled" tabindex="' . ($TabIndex++) . '" type="text" size="1" maxlength="3" name="d_t_' . $MyRow->TfEvent . '_' . $MyRow->GrMatchNo . '_' . $i . '" id="d_t_' . $MyRow->TfEvent . '_' . $MyRow->GrMatchNo . '_' . $i . '" value="' . DecodeFromLetter($TieBreak[$i]) . '" onBlur="javascript:SendTieBreak(\'d_t_' . $MyRow->TfEvent . '_' . $MyRow->TfMatchNo . '\',' . ($obj->so) . ');" disabled>';
							}
		...
		// Adesso gestisco l'oro e il bronzo
			...
			// righe di testa della fase
				...
				// righe mediane ogni due arcieri
					...
					$MyGrid[$Row][$Col].= '<td nowrap class="Center ' . $TipoBordo . '">&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;</td>';
				// Atleta
					$MyGrid[$Row][$Col].= '<td nowrap class="Bottom Top Right Left"><div id="idAth_' . $MyRow->TfEvent . '_' . $MyRow->GrMatchNo . '">' . (!is_null($MyRow->TeamName) ? $MyRow->TeamName : '&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;') /*. ' ' . $MyRow->GrMatchNo*/ . '</div></td>';
				// Codice Nazione (o bandiera)
					$MyGrid[$Row][$Col].= '<td nowrap class="Bottom Top Right Left"><div id="idCty_' . $MyRow->TfEvent . '_' . $MyRow->GrMatchNo . '">' . (!is_null($MyRow->CoCode) ? $MyRow->CoCode : '&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;') . '</div></td>';
				// Punteggio
				...
				// tie
					...
					if (isset($_REQUEST['d_Tie']) && $_REQUEST['d_Tie']==1)
					{
						...
						for ($i=0;$i<$obj->so;++$i)
						{
							$MyGrid[$Row][$Col].= '<input type="text"  class="disabled" tabindex="' . ($TabIndex++) . '" size="1" name="d_t_' . $MyRow->TfEvent . '_' . $MyRow->GrMatchNo . '_' . $i . '" id="d_t_' . $MyRow->TfEvent . '_' . $MyRow->GrMatchNo . '_' . $i . '" value="' . DecodeFromLetter($TieBreak[$i]) . '"  onBlur="javascript:SendTieBreak(\'d_t_' . $MyRow->TfEvent . '_' . $MyRow->TfMatchNo . '\',' . ($obj->so) . ');" disabled>';
						}
						$MyGrid[$Row][$Col].= '<br/><input list="NoteList" value="'.$MyRow->TfNotes.'" tabindex="' . ($TabIndex++) . '" name="d_N_' . $MyRow->TfEvent . '_' . $MyRow->GrMatchNo . '" id="d_N_' . $MyRow->TfEvent . '_' . $MyRow->GrMatchNo . '" onChange="javascript:SendToServer(\'d_N_' . $MyRow->TfEvent . '_' . $MyRow->GrMatchNo . '\');">' . "\n";
					}
					else
