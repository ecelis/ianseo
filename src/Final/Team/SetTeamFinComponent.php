<?php
	...
	$query
		= "SELECT "
			. "EnId AS `id`,EnDivision AS `div`,EnClass AS `class`,EnAgeClass AS `ageClass`,EnCode AS `code`,CONCAT(EnFirstName,' ',EnName) AS `name` "
		. "FROM "
			. "Entries "
			. "INNER JOIN "
				. "EventClass "
			. "ON EcCode=" . StrSafe_DB($ev) . " AND EcTeamEvent!=0 AND EcTournament=" . StrSafe_DB($_SESSION['TourId']) . " AND EcClass=EnClass AND EcDivision=EnDivision "
		. "WHERE "
			. "EnTournament=" . StrSafe_DB($_SESSION['TourId']) . " AND IF(EnCountry2=0,EnCountry,EnCountry2)=" . StrSafe_DB($team) . " AND EnAthlete=1 AND EnStatus<=1 "
			. "AND EnId NOT IN("
				. "SELECT "
					. "TfcId "
				. "FROM "
					. "TeamFinComponent "
				. "WHERE "
					. "TfcTournament=" . StrSafe_DB($_SESSION['TourId']) . " AND TfcEvent=" . StrSafe_DB($ev) . " "
			. ")";
	//print $query;exit;
