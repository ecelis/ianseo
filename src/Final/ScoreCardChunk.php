...
$scores=array(1=>'','');
$Alternate=($myRow->first1 or $myRow->first2);
$ClassAlternate=($Alternate ? ' class="ShootFirst"' : ' class="ShootFirst hidden"'); // if match was alternate at least one of those is > 0
...
for($m=1;$m<=2;++$m) {
	$out='';
	if ($myRow->{'name' . $m}!='***') {
		$out.='<table class="Tabella" id="matchtab'.$m.'">' . "\n";
		$out.='<tr><th colspan="' . (6+$cols). '">' . $myRow->{'name' .$m} .'</th></tr>' . "\n";
		...
		// header score
			$out.='<tr>';
				$out.='<th'.$ClassAlternate.'>'.get_text('ShootsFirst', 'Tournament').'</th>';
				$out.='<th></th>';
				for ($i=0;$i<$cols;++$i)
				{
					$out.='<th>' . ($i+1) . '</th>';
		...
		// righe
			...
			$TabIndex=100;
			for ($i=0;$i<$rows;++$i) {
				$ShootsFirst=($myRow->{'first'.$m} & pow(2, $i)); // is this archer shooting this end first?
				$pr=0;		// totale progressivo
				$prValues=true;	//Vede se ci sono valori
				$out.='<tr>';

				// inserts the cell for shooting first
				$out.='<th'.$ClassAlternate.'><input type="radio" id="first['.$team.']['.$myRow->event.']['.$myRow->{'match' .$m}.']['.$i.']" name="first['.$i.']" onclick="setShootingFirst(this)"'.($ShootsFirst ? ' checked="checked"' : '').'></th>';
					$out.='<th>' . ($i+1) . '</th>';
				...
				// celle di input
					 ...
					 * s_match_arrowIndex
					 */
						if(!empty($myRow->{'arrowString'.$m}) && isset($myRow->{'arrowString'.$m}[$idx]) && array_key_exists($myRow->{'arrowString'.$m}[$idx] , $validData["Arrows"])) {
...
if($chunkMode == 0 || $chunkMode == 1) {
	...
	echo '&nbsp;&nbsp;&nbsp;<input type="checkbox" id="alternate" onclick="ToggleAlternate()"'.($Alternate ? ' checked="checked"' : '').'/>'.get_text('AlternateMatch', 'Tournament');
