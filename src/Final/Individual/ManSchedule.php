<?php
	...
	$JS_SCRIPT=array(
		...
		'<style>.ScheduleSelect {max-width:15em;overflow:hidden;}</style>',
		);
	$PAGE_TITLE=get_text('ManFinScheduleInd');
...
<table class="Tabella">
<tr><th class="Title" colspan="2"><?php print get_text('ManFinScheduleInd'); ?></th></tr>
		...
		*/
		...
		// Il ciclo gestisce fino alla semifinale
			...
			// Estraggo la griglia della fase $CurPhase
				$Select
					= "SELECT GrPhase,IF(EvFinalFirstPhase=48 || EvFinalFirstPhase=24,GrPosition2, GrPosition) as GrPosition,GrMatchNo,	/* Grids*/ "
					. "FinEvent, /* Finals*/"
					...
					. "LEFT JOIN FinSchedule ON FinEvent=FSEvent AND FinMatchNo=FSMatchNo AND (FSTeamEvent='0' OR FSTeamEvent IS NULL) AND FinTournament=FSTournament "
				...
				// righe di testa della fase
					for ($i=0;$i<=$HeadRows+2;++$i)
					{
						...
						$ColspanDtHr = ($CurPhase==$StartPhase || ($StartPhase==24 && $CurPhase==32) || ($StartPhase==48 && $CurPhase==64) ? '1' : '1');
						$Input
							= '<input type="text" maxlength="10" size="10" id="d_FSScheduledDateAll_' . $_REQUEST['d_Event'] . '_' . $CurPhase . '" value="">@'
							. '<input type="text" maxlength="5" size="4" id="d_FSScheduledTimeAll_' . $_REQUEST['d_Event'] . '_' . $CurPhase . '" value="">'
							. (!defined('hideSchedulerAndAdvancedSession') ? '&nbsp;/&nbsp;<input type="text" maxlength="3" size="2" id="d_FSScheduledLenAll_' . $_REQUEST['d_Event'] . '_' . $CurPhase . '" value="">' : '')
							. '<br>'
						...
						if ($i==1) {
							...
							$Txt.= $Input;
							$Txt.= get_already_scheduled_events($CurPhase, $_REQUEST['d_Event']);
							$Txt.= '</td>';
						} else {
					...
					// data/ora
						if ($AthPrinted==0)
						{
							...
							$MyGrid[$Row][$Col]
								...
								. '<input type="text" tabindex="' . ($TabIndex++) . '" maxlength="10" size="10" name="d_FSScheduledDate_' . $MyRow->FinEvent . '_' . $MyRow->GrMatchNo . '" id="d_FSScheduledDate_' . $MyRow->FinEvent . '_' . $MyRow->GrMatchNo . '" value="' . $Dt . '" onBlur="javascript:WriteSchedule(\'d_FSScheduledDate_' . $MyRow->FinEvent . '_' . $MyRow->GrMatchNo . '\');">@'
								. '<input type="text" tabindex="' . ($TabIndex++) . '" maxlength="5" size="4" name="d_FSScheduledTime_' . $MyRow->FinEvent . '_' . $MyRow->GrMatchNo . '" id="d_FSScheduledTime_' . $MyRow->FinEvent . '_' . $MyRow->GrMatchNo . '" value="' . $Hr . '"  onBlur="javascript:WriteSchedule(\'d_FSScheduledTime_' . $MyRow->FinEvent . '_' . $MyRow->GrMatchNo . '\');">'
								. (!defined('hideSchedulerAndAdvancedSession') ? '&nbsp;/&nbsp;<input type="text" tabindex="' . ($TabIndex++) . '" maxlength="3" size="2" name="d_FSScheduledLen_' . $MyRow->FinEvent . '_' . $MyRow->GrMatchNo . '" id="d_FSScheduledLen_' . $MyRow->FinEvent . '_' . $MyRow->GrMatchNo . '" value="' . $mLn . '"  onBlur="javascript:WriteSchedule(\'d_FSScheduledLen_' . $MyRow->FinEvent . '_' . $MyRow->GrMatchNo . '\');">' : '')
								. '</td>';
						}
					...
					// ogni due arcieri stampo le righe mediane
						if ($AthPrinted==2 && $TotAthPrinted!=($StartPhase==48 ? 128 : ($StartPhase==24 ? 64 : $CurPhase*2)))
						{
		...
		// oro/bronzo
			...
			// righe di testa della fase
				...
				// se sto stampando l'ultima riga di testa scrivo la fase
					...
					elseif ($i==2)
						$Txt = '<td colspan="2">&nbsp;</td><td nowrap class="" colspan="' . ($Colspan-2) . '">'
							. $Input
							. get_already_scheduled_events(1, $_REQUEST['d_Event'])
							. '</td>';
					else
				...
				// data/ora
						...
						$MyGrid[$Row][$Col]
							.='<td nowrap rowspan="2" class="Bottom Top Right Left">'
							. '<input type="text" tabindex="' . ($TabIndex++) . '" maxlength="10" size="10"  id="d_FSScheduledDate_' . $MyRow->FinEvent . '_' . $MyRow->GrMatchNo . '" value="' . $Dt . '" onBlur="javascript:WriteSchedule(\'d_FSScheduledDate_' . $MyRow->FinEvent . '_' . $MyRow->GrMatchNo . '\');">@'
							. '<input type="text" tabindex="' . ($TabIndex++) . '" maxlength="5" size="4" id="d_FSScheduledTime_' . $MyRow->FinEvent . '_' . $MyRow->GrMatchNo . '" value="' . $Hr . '" onBlur="javascript:WriteSchedule(\'d_FSScheduledTime_' . $MyRow->FinEvent . '_' . $MyRow->GrMatchNo . '\');">'
							. (!defined('hideSchedulerAndAdvancedSession') ? '&nbsp;/&nbsp;<input type="text" tabindex="' . ($TabIndex++) . '" maxlength="5" size="2" name="d_FSScheduledLen_' . $MyRow->FinEvent . '_' . $MyRow->GrMatchNo . '" id="d_FSScheduledLen_' . $MyRow->FinEvent . '_' . $MyRow->GrMatchNo . '" value="' . $mLn . '"  onBlur="javascript:WriteSchedule(\'d_FSScheduledLen_' . $MyRow->FinEvent . '_' . $MyRow->GrMatchNo . '\');">' : '')
							. '</td>';
					}
					++$AthPrinted;
					++$TotAthPrinted;
