...
if(!empty($GoBack)) {
    ...
    echo '</td></tr>';
    echo '<tr><td colspan="3" class="Opponents CmdRow">';
    echo '<div style="display: inline-block; width: 50%; text-align: left;"><input type="checkbox" id="MatchAlternate" onclick="toggleAlternate()"/>'.get_text('AlternateMatch', 'Tournament').'</div>'.
		'<div style="display: inline-block; width: 50%; text-align: right;"><input type="button" id="moveWinner" onclick="moveToNextPhase(this)" value="'.get_text('MoveWinner2NextPhase','Tournament').'"></div>';
    echo '<div><input type="button" id="liveButton" value="" onclick="setLive()"/></div>';
    echo '</td></tr>';
