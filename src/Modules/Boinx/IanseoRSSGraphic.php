...
$FlagTpl='TV/Photos/' . $TourCodeSafe . '-Fl-%s.jpg';
$FlagSvg='TV/Photos/' . $TourCodeSafe . '-FlSvg-%s.svg';
$fotodir='http://' . $_SERVER['HTTP_HOST'] . $CFG->ROOT_DIR ;
...
if($Ind) {
	...
	if(!$options['cutRank']) unset($options['cutRank']);
	$family='DivClass';
...
//	$family = 'Snapshot';
	...
	foreach($rankData['sections'] as $Event => $data) {
		...
		if($Ind[$Event]=='al') $Ind[$Event]=count($data['items']);
...
//	$options['subFamily'] = 'Abs';
	...
	foreach($rankData['sections'] as $Event => $data) {
		...
		$txt=array();
		if($AbsInd[$Event]=='al') $AbsInd[$Event]=count($data['items']);
...
$XmlRss = $XmlDoc->createElement('globalrank');
...
function getTeamItems($Team) {
	...
	while($MyRow=safe_fetch($Rs)) {
		...
		if($Team[$MyRow->TeEvent]=='al' or $MyRank<=$Team[$MyRow->TeEvent]) {
