 ...
 * Estende Obj_Rank
 */
	class Obj_Rank_FinalInd_3_LooserBrackets_calc extends Obj_Rank_FinalInd
	{
	/**
	 * writeRow()
	 ...
	 */
		...
		// reset delle RankFinal della fase x le persone di quell'evento e quella fase
			...
			// Seed the highest rank possible based on the event
			$tmp=explode('-', $event);
			$tt=safe_r_sql("select EvFinalFirstPhase from Events WHERE EvTournament={$this->tournament} AND EvCode='{$tmp[0]}' ");
			$uu=safe_fetch($tt);
			$StartingPhase=$uu->EvFinalFirstPhase;
			if(empty($tmp[1])) {
				$Main=true;
				$StartRank=1;
			} else {
			...
			if ($rs)
			{
				 ...
				 */
					if ($phase==0 || $phase==1)
					{
						...
						{
						// vincente
// 							$toWrite[]=array('event'=>$event,'id'=>$myRow->OppAthId,'rank'=> $StartRank);
							$toWrite[]=array('event'=>$tmp[0],'id'=>$myRow->OppAthId,'rank'=> $StartRank);
						// perdente
// 							$toWrite[]=array('event'=>$event,'id'=>$myRow->AthId,'rank'=> $StartRank+1);
							$toWrite[]=array('event'=>$tmp[0],'id'=>$myRow->AthId,'rank'=> $StartRank+1);
						}
						elseif ($phase==1)
						{
						// vincente
// 							$toWrite[]=array('event'=>$event,'id'=>$myRow->OppAthId,'rank'=> $StartRank+2);
							$toWrite[]=array('event'=>$tmp[0],'id'=>$myRow->OppAthId,'rank'=> $StartRank+2);
						// perdente
// 							$toWrite[]=array('event'=>$event,'id'=>$myRow->AthId,'rank'=> $StartRank+3);
							$toWrite[]=array('event'=>$tmp[0],'id'=>$myRow->AthId,'rank'=> $StartRank+3);
						}
					...
					elseif ($phase<=4 or ($phase<=8 and $StartingPhase==8) or !$Main)
					{
					// non faccio nulla!
					}
					else
