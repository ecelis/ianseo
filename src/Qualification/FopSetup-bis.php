...
// select assigned number of sessions with min and max target
$SesSql=safe_r_sql("select group_concat(distinct QuSession order by QuSession) QuSession, group_concat(distinct if(SesName>'', concat(QuSession, ' - ', SesName), QuSession) order by QuSession) Sessions, min(cast( substr(QuTargetNo,2) as unsigned)) TargetMin, max(cast( substr(QuTargetNo,2) as unsigned)) TargetMax, ToNumDist
	from Qualifications
	inner join Entries on EnId=QuId
	inner join Tournament on EnTournament=ToId
	left join Session on QuSession=SesOrder and SesTournament={$_SESSION['TourId']}
	where EnTournament={$_SESSION['TourId']} and cast( substr(QuTargetNo,2) as unsigned)>0
	group by ToNumDist");
...
while($SesRow=safe_fetch($SesSql)) {
	...
	$FirstTarget=$SesRow->TargetMin;

	$SesNames=explode(',', $SesRow->Sessions);

	// prints the session
	foreach(explode(',', $SesRow->QuSession) as $k=>$Session) {
		...
		$pdf->Cell($pdf->GetPageWidth()-20, 8, get_text('Session') . " " . $SesNames[$k], 0, 0, "C", 1);
		$TopPos+=8;
		...
		for($n=1; $n<=$SesRow->ToNumDist; $n++) {
			...
			$Sql="select distinct cast(substr(QuTargetNo,2) as unsigned) TargetNo, IFNULL(Td$n,'.$n.') as Distance, TarDescr, TarDim, DiDay, DiStart, DiWarmStart from
				Entries
				...
				left join DistanceInformation on QuSession=DiSession and DiTournament={$_SESSION['TourId']} and DiDistance=$n
			...
			while($r=safe_fetch($q)) {
				...
				if($r->DiDay!='0000-00-00') {
					$Day = ' ('.$r->DiDay;
					if($r->DiStart!='00:00:00') {
						$Day.=' '.substr($r->DiStart, 0, 5);
					}
					$Day.= ')';
				}
			...
			$pdf->Cell($pdf->GetPageWidth()-20,7, get_text('Distance','Tournament') . ": " . $n . $Day, 0, 0, "C", 1);
			$TopPos+=8;
				...
				foreach($TarTypes as $TarDesc => $TarDims) {
					foreach($TarDims as $TarDim => $Groups) {
						foreach($Groups as $Targets) {
							...
							$pdf->SetTextColor(0);
							$pdf->SetXY(10+ColName+($DimTarget*($Targets[0]-$FirstTarget)), $TopPos);
							...
							$pdf->SetXY(10+ColName+($DimTarget*($Targets[0]-$FirstTarget)), $TopPos+4);
							$pdf->Cell($DimTarget*(1+$Targets[1]-$Targets[0]), 4, get_text($TarDesc),'TLR',0,"C");
							$pdf->SetXY(10+ColName+($DimTarget*($Targets[0]-$FirstTarget)), $TopPos+8);
