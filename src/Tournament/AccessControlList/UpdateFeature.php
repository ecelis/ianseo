...
$Json = array();
if(!empty($_REQUEST["IP"]) AND $ip = filter_var($_REQUEST["IP"], FILTER_VALIDATE_IP) AND $ip!='127.0.0.1') {
...
if(!empty($_REQUEST["deleteIP"]) AND $ip = filter_var($_REQUEST["deleteIP"], FILTER_VALIDATE_IP)) {
...
if(isset($_REQUEST["featureIP"]) AND $ip = filter_var($_REQUEST["featureIP"], FILTER_VALIDATE_IP) AND isset($_REQUEST["levelID"]) AND preg_match("/^[0-2]{1}$/",$_REQUEST["levelID"])) {
...
if(isset($_REQUEST["featureIP"]) AND $ip = filter_var($_REQUEST["featureIP"], FILTER_VALIDATE_IP) AND isset($_REQUEST["featureID"]) AND preg_match("/^[0-9]+$/",$_REQUEST["featureID"]) AND ($feature = intval($_REQUEST["featureID"]))<count($listACL)) {
    $lvl = 0;
    ...
    if($lvl++ == 2 OR (array_key_exists($feature, $limitedACL) AND $lvl > $limitedACL[$feature])) {
