 ...
 */
	...
	if($_POST) {
		...
		$RET->PDF=array();
		$ShowRecords=isset($_POST['showRecords']);
		...
		if(empty($_POST['btnDelOnline'])) {
			...
			// List by targets
			if(!empty($_POST['ENE'])) $RET->ENE=getStartListCategory($ORIS, 1);
			...
			if(!empty($_POST['QualificationInd'])) {
				$RET->IQ=new StdClass();
				foreach($_POST['QualificationInd'] as $Event) $RET->IQ->{$Event} = getQualificationIndividual(substr($Event,2), $ORIS, $ShowRecords);
			}
			...
			if(!empty($_POST['QualificationTeam'])) {
				$RET->TQ=new StdClass();
				foreach($_POST['QualificationTeam'] as $Event) $RET->TQ->{$Event}=getQualificationTeam(substr($Event,2),$ORIS, $ShowRecords);
			}

			// Brackets, Individual
			if(!empty($_POST['BracketsInd'])) {
				$RET->IB=new StdClass();
				foreach($_POST['BracketsInd'] as $Event) $RET->IB->{$Event}=getBracketsIndividual(substr($Event,2),$ORIS, true, true, true, $ShowRecords);
			}

			// Brackets, Team
			if(!empty($_POST['BracketsTeam'])) {
				$RET->TB=new StdClass();
				foreach($_POST['BracketsTeam'] as $Event) $RET->TB->{$Event}=getBracketsTeams(substr($Event,2),$ORIS, true, true, true, $ShowRecords);
			}
			...
			if(!empty($_POST['ENE'])) $RET->delete[]='ENE'; // List by targets
	...
	<div class="medium">
		<table class="Tabella">
			...
			<tr>
				...
				<td class="Right">
				<?php print get_text('showRecords','Tournament'); ?>
				<input name="showRecords" type="checkbox" id="showRecords" <?php echo (empty($_POST['showRecords']) ? '' : 'checked="checked"'); ?> />
				&nbsp;
...
<!-- StartList -->
			<tr>
				<td class="Bold Center"><input type="checkbox" value="STE" name="STE" id="STE" />&nbsp;<?php print get_text('StatEvents','Tournament'); ?></td>
				<td class="Bold Center" colspan="2"><input type="checkbox" value="IMG" name="IMG" id="IMG" <?php echo empty($_SESSION['SendOnlinePDFImages']) ? '' : 'checked'; ?>/>&nbsp;<?php print get_text('SendLogos','Tournament') ?></td>
				<td class="Bold Center"><input type="checkbox" value="STC" name="STC" id="STC" />&nbsp;<?php echo get_text('StatCountries','Tournament'); ?></td>
			</tr>
			<tr>
				<td class="Bold Left"><input type="checkbox" value="ENE" name="ENE" id="ENE" />&nbsp;<?php print get_text('StartlistTeam','Tournament') ?></td>
