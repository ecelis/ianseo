...
//Immagine di Destra
			if(isset($_FILES) && array_key_exists('UploadedFileR',$_FILES) &&  $_FILES['UploadedFileR']['error']==UPLOAD_ERR_OK && $_FILES['UploadedFileR']['size']>0 && $_FILES['UploadedFileR']['size']<=65535)
			{
...
//Immagine in basso
			if(isset($_FILES) && array_key_exists('UploadedFileB',$_FILES) &&  $_FILES['UploadedFileB']['error']==UPLOAD_ERR_OK && $_FILES['UploadedFileB']['size']>0 && $_FILES['UploadedFileB']['size']<=65535)
			{
...
<?php
	...
	{
		echo '<input type="hidden" name="MAX_FILE_SIZE" value="65535"><input name="UploadedFileL" type="file" size="50" />';
	}
?>
	<br/>
	</td>
...
<?php
	...
	{
		echo '<input type="hidden" name="MAX_FILE_SIZE" value="65535"><input name="UploadedFileR" type="file" size="50" />';
	}
?>
	<br/>
	</td>
...
<?php
	...
	{
		echo '<input type="hidden" name="MAX_FILE_SIZE" value="65535"><input name="UploadedFileB" type="file" size="50" />';
	}
?>		</td>
</tr>
<tr class="Divider"><td colspan="3"></td></tr>
