	...
	if ($error==0)
	{
		$query
			= "UPDATE "
				. "Entries "
			. "SET "
				. "EnCountry2=" . StrSafe_DB($coId) . " "
			. "WHERE "
				. "EnId=" . StrSafe_DB($id) . " ";
		$rs=safe_w_sql($query);
		if($EnSelect=GetAccBoothEnWhere($id, true, true)) {
			LogAccBoothQuerry("UPDATE Entries SET EnCountry2=(select CoId from Countries were CoCode='$countryCode' and CoTournament=§TOCODETOID§) where $EnSelect");
		}
	}
