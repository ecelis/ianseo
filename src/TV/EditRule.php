...
if($_POST) {
	...
	if(!empty($_POST['d_TVPage'])) {
		...
		// updates the settings of rules and page
		safe_w_sql("update TVRules SET ".
				"TVRSettings=" . strsafe_DB(serialize($_POST['R-Main'])).
				" WHERE TVRId=$RuleId AND TVRTournament=$TourId");
		if(isset($_POST['P-Main'])) $params[]="TVPSettings=".strsafe_DB(serialize($_POST['P-Main']));
