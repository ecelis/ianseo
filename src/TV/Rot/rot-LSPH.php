...
// 	CheckPictures($IsCode);
	...
	$ret[]='<div class="Title">'.get_text('StartlistSessionPicture','Tournament').'</div>';
	...
	while($MyRow=safe_fetch($Rs)) {
		...
		if($OldTarget!=intval($MyRow->TargetNo)) {
			...
			if($tab) {
				...
				$tmp= '<div class="TgtBlock">';
				foreach($tab as $letter=>$r) {
					$tmp.='<div class="TgtAssign Font2'.$Class.' Back2'.$Class.'">';
...
function getPageDefaults(&$RMain) {
	...
	$ret=array(
			...
			'FlagDiv' => 'flex: 0 0 3.95rem;margin-right:0.2rem;height:3rem;',
			'Flag' => 'height:2.5rem; border:0.1rem solid #888;',
			'Target' => 'flex: 0 0 7rem; font-size:1.25em; text-align:right;',
			'Athlete' => 'flex: 1 1; font-weight:bold; overflow:hidden; width:100%; white-space:nowrap;margin: 0 0.2em;',
			'CountryDescr' => 'text-align:left; width:auto;',
			'Category' => 'flex: 1 0; font-size:0.8em; overflow:hidden; width:100%; white-space:nowrap;margin: 0 0.2em;',
