...
function UpdateTournament($Gara) {
	...
	if($Gara['Tournament']['ToDbVersion']<'2010-04-17 15:34:01') {
		if(empty($Gara['TournamentDistances'])) {
			...
			$t=safe_w_sql("SELECT distinct td.* FROM TournamentDistances td inner join TempClasses e on TcClass LIKE TdClasses where TdType='{$Gara['Tournament']['ToType']}' and TdTournament=0");
...
//		$Gara['Tournament']['ToDouble']='0';
	...
	if($Gara['Tournament']['ToDbVersion']<'2016-02-05 18:10:00') {
		unset($Gara['TVOut']);
	}
