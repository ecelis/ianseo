			...
			*/
			...
			if($this->HasArchers) {
				...
				$SQL[]="select distinct
					...
					from (select EnTournament, concat(EnDivision,EnClass) as Category, QuSession
						from Entries
						inner join Qualifications on QuId=EnId
						Where EnTournament=$this->TourId
						group by EnDivision, EnClass, QuSession) Entries
					STRAIGHT_JOIN Session on SesTournament=EnTournament and SesOrder=QuSession and SesType='Q'
					STRAIGHT_JOIN TournamentDistances on TdTournament=EnTournament and Category like TdClasses
					STRAIGHT_JOIN DistanceInformation on DiTournament=EnTournament and DiSession=SesOrder and DiType=SesType and DiDay>0 and (DiStart>0 or DiWarmStart>0)"
