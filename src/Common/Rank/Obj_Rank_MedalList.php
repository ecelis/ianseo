		...
		if (isset($this->opts['divcl']['i']) && $this->opts['divcl']['i']==true)
		{
			$queries[]="
				(
					...
					QuClRank as Rank, QuScore, CoCode, CoName, CONCAT_WS(',',EnCode,EnFirstName,EnName, EnNameOrder) as Athlete, ToWhenTo as Date,
		...
		if (isset($this->opts['divcl']['t']) && $this->opts['divcl']['t']==true)
		{
			$queries[]="
				(
					...
					GROUP_CONCAT(CONCAT_WS(',',EnCode,EnFirstName,EnName, EnNameOrder) ORDER BY EnFirstName SEPARATOR '|') as Athlete, ToWhenTo as Date,
		...
		if (!isset($this->opts['final']['i']) || $this->opts['final']['i']==true)
		{
			$queries[]="
				(
					...
					IF(EvFinalFirstPhase!=0, IndRankFinal, IndRank) as Rank, QuScore, CoCode, CoName, CONCAT_WS(',',EnCode,EnFirstName,EnName, EnNameOrder) as Athlete,
		...
		if (!isset($this->opts['final']['t']) || $this->opts['final']['t']==true)
		{
			$queries[]="
				(
					...
					GROUP_CONCAT(IF(EvFinalFirstPhase!=0, CONCAT_WS(',',ef.EnCode,ef.EnFirstName,ef.EnName, ef.EnNameOrder),CONCAT_WS(',',eq.EnCode,eq.EnFirstName,eq.EnName, eq.EnNameOrder)) ORDER BY ef.EnFirstName,eq.EnFirstName SEPARATOR '|') as Athlete,
